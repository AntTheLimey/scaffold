...
| Per-iteration budget cap for CLI specialists | #6 | $2 max_budget_usd on all CLI specialists in agents.yaml |
| Specialist selection by task domain | #6 | Architect declares specialist + file_paths; developer uses cascading priority: architect_specialist → architect_file_paths → regex → roster → languages → fallback |
| Timeout cost recovery | #6 | parse_cli_output accumulates tokens from streamed assistant messages; ralph_loop catches TimeoutExpired and recovers cost from partial stdout |
